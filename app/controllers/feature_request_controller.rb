  ...
  def list
    ...
    respond_to do |format|
      format.html do
        ...
        if request.xhr?
          render :partial => 'fr_list', :layout => false
        else
          render
        end
      end
      format.xml do
        @feature_requests = @project.feature_requests.where(@conditions).order(params[:sort_by])
        render :xml => @feature_requests
      end
      format.json do
        @feature_requests = @project.feature_requests.where(@conditions).order(params[:sort_by])
        render :json => @feature_requests
      end
    end
  end
