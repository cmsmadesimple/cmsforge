namespace :db do      
    ...
    task(:dump => :environment) do
      ...
      unless db_config['password'].blank?
        ...
        sh "mysqldump -u #{db_config['username']} -Q --add-drop-table --add-locks #{db_config['database']} > #{RAILS_ROOT}/db/#{db_config['database']}.sql"     
      end
    end
    ...
    task(:load => :environment) do
      ...
      unless db_config['password'].blank?
        ...
        sh "mysql -u #{db_config['username']} #{db_config['database']} <  #{RAILS_ROOT}/db/#{db_config['database']}.sql"
      end
    end
end
