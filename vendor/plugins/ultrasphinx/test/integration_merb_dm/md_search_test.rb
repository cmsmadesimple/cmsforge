...
class MdSearchTest < Test::Unit::TestCase
  ...
  def test_query_retries_and_fails
    system("cd #{Merb.root}; rake ultrasphinx:daemon:stop &> /dev/null")
    assert_raises(Ultrasphinx::DaemonError) do
      S.new.run
    end
  ensure
    # In case this test fails, we want to be sure it doesn't cause all the later tests to blow up too
    system("cd #{Merb.root}; rake ultrasphinx:daemon:start &> /dev/null")
    sleep(1)
  end
  ...
  def test_conditions
    @deleted_count = User.count(:deleted => true)
...
# Sorting
  def test_sort_by_date
    assert_equal(
      Seller.all(:limit => 5, :order => [:created_at.desc]).map(&:created_at),
  ...
  def test_sort_by_float
    assert_equal(
      Seller.all(:limit => 5, :order => [:capitalization]).map(&:capitalization),
  ...
  def test_sort_by_string
    assert_equal(
      Seller.all(:limit => 5, :order => [:mission_statement]).map(&:mission_statement),
  ...
  def test_sort_by_string_using_query
    assert_equal(
      Seller.all(:id => [10,11], :order => [:mission_statement]).map(&:mission_statement),
...
# Filters
  ...
  def test_float_range_filter
    @count = Seller.count(:capitalization.gte => 10.0, :capitalization.lte => 29.5)
    assert_equal(@count,
  ...
  def test_date_range_filter
    @first, @last = Seller.get(5).created_at, Seller.get(10).created_at
    @items = Seller.all(:created_at.gte => @last, :created_at.lte => @first).sort_by(&:id)
    @count = @items.size
  ...
  def test_text_filter
    assert_equal(
      Seller.count(:company_name => 'seller17'),
...
# Facets
  ...
  def test_numeric_facet
    @user_id_count = Seller.all.map(&:user_id).uniq.size
