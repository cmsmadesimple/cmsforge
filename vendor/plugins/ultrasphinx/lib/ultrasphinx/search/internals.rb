    ...
    module Internals
      ...
      def rebuild_facet_cache(facet)
        ...
        configured_classes = Fields.instance.classes[facet].map do |klass|
          ...
          klass.connection.get_facets("SELECT #{field_string} AS value, #{SQL_FUNCTIONS[ADAPTER]['hash']._interpolate(field_string)} AS hash FROM #{klass.table_name} #{join_string} GROUP BY value").each do |value, hash|
      ...
      def convert_sphinx_ids(sphinx_ids)    
        ...
        raise ConfigurationError, "No model mappings were found. Your #{ENV}.conf file is corrupted, or your application container needs to be restarted." if number_of_models == 0
