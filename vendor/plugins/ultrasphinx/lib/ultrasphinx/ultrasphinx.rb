...
module Ultrasphinx
  ...
  CONF_PATH = "#{DIR}/#{ENV}.conf"
  
  ENV_BASE_PATH = "#{DIR}/#{ENV}.base" 
  
  GENERIC_BASE_PATH = "#{DIR}/default.base"
  
  BASE_PATH = (File.exist?(ENV_BASE_PATH) ? ENV_BASE_PATH : GENERIC_BASE_PATH)
  
  raise ConfigurationError, "Please create a '#{SUBDIR}/#{ENV}.base' or '#{SUBDIR}/default.base' file in order to use Ultrasphinx in your #{ENV} environment." unless File.exist? BASE_PATH # XXX lame
  ...
  def self.get_models_to_class_ids #:nodoc:
    ...
    if File.exist? CONF_PATH
      ...
      Ultrasphinx.say "configuration file not found for #{ENV.inspect} environment"
